...
var debug = require('debug')('seed_with_multiple_connections');
...
function promiseBuffer(n) {
  return new Promise((resolve, reject) => {
    debug('values promised to be buffered. Meanwhile setup connection');
    ...
    resolve({'bufferedValues': bufVals, 'connection': conn});
  });
}
...
for (var i = 0; i < t; i++) {
  promiseBuffers[i] = promiseBuffer(b)
    .then((pb) => {
      ...
      var statement = 'INSERT INTO student(id, first, last, dob, lane, heldback, entrance) VALUES ?';
      // eval(pry.it);
      pb['connection'].query(statement, [pb['bufferedValues']], function(err, res) {
        if(err) throw err;
        debug('Last insert ID: ', res.insertId, ' affected rows: ', res.affectedRows);
      });
    });
};
